...
import Move from '../../../openxum-core/games/linesofactions/move.mjs';
import Coordinates from '../../../openxum-core/games/linesofactions/coordinates.mjs';
...
class Gui extends OpenXum.Gui {
    ...
    set_canvas(c) {
        ...
        this._offsetX = this._canvas.width / 2 - this._deltaX * 4.0;
...
//dessine les pieces
    _draw_piece(x, y, piece) {
        ...
        if (piece.color() === LinesOfActions.Color.BLACK) {
    ...
    _draw_selected_piece() {
        let x = this._selected_piece.coordinates().x();//recup coord piece
        let y = this._selected_piece.coordinates().y();
        let possible_moves = this._engine._get_possible_move_list(this._selected_piece);//recup coup possible
        let pt = this._compute_coordinates(x, y);
        let radius = (this._deltaX / 2.3);

        this._context.lineWidth = 4;//epaisseur du contour lors de la selection
        this._context.strokeStyle = "#d8370f";
        this._context.beginPath();//demarrer ligne
        this._context.arc(pt[0], pt[1], radius, 0.0, 2 * Math.PI);//dessin cercle
        this._context.closePath();//fin ligne
        this._context.stroke();

        this._context.fillStyle = "#d8370f";//interieur couleur
        ...
        for(let i = 0; i < possible_moves.length; i++) {
            ...
            this._context.beginPath();
            this._context.arc(pt[0], pt[1], radius, 0.0, 2 * Math.PI);//dessin des cercles rempli de rouge qui pointe les cases possibles.
            this._context.closePath();
            this._context.fill();
        }
    }
//dessine la grille.
    _draw_grid() {
        ...
        //this._context.lineWidth = 1;//aucune utilité
        //this._context.strokeStyle = "#000000";//aucune utilité
        this._context.fillStyle = "#d59d67";//couleur fond
        for (i = 0; i < 8; ++i) {
            for (j = 0; j < 8; ++j) {
                this._context.beginPath();
...
//creer des croix dans la case du milieu est dans le coin sup gauche isfortress() ok si doit placer croix
                ...
                //cela dessine le petit roi dans la piece blanche
                        /*if (piece.isKing()) {
                            this._context.strokeStyle = "#303030";
                            this._context.fillStyle = "#303030";
                            this._context.beginPath();
