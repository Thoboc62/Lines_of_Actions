...
class Gui extends OpenXum.Gui {
    ...
    draw() {
        ...
        // background
        this._context.fillStyle = "#800000";
    ...
    _animate(val, deplacement) {
        ...
        if (continueAnimate) {
            ...
            let that = this;
            setTimeout(function() {
                that._animate(val*2 + 6, deplacement);
            }, 10);
    ...
    _animate_move() {
        ...
        if (from.x() < to.x() && from.y() > to.y()) deplacement = 6;
        if (from.x() > to.x() && from.y() > to.y()) deplacement = 7;
        if (from.x() > to.x() && from.y() < to.y()) deplacement = 5;
    ...
    _draw_piece(x, y, piece) {
        ...
        if (piece.color() === LinesOfActions.Color.WHITE) {
            this._context.strokeStyle = "#FFFAF0";
            this._context.fillStyle = "#FFFAF0";
        }
        else {
    ...
    _draw_selected_piece() {
        if (this._selected_piece===undefined){
            console.log("pas de piece");}
        else{ console.log(this._selected_piece);}
        ...
        this._context.strokeStyle = "#FFD700";
        this._context.beginPath();
...
//dessine la grille.
    _draw_grid() {
        ...
        for (i = 0; i < 8; ++i) {
            for (j = 0; j < 8; ++j) {
                if(i%2===0 ){
                    if(j%2===0){this._context.fillStyle = "#D2B48C";}
                    else {this._context.fillStyle = "#CD853F";}
                }
                else {
                    if(j%2===0){this._context.fillStyle = "#CD853F";}
                    else {this._context.fillStyle = "#D2B48C";}
                }
                this._context.beginPath();
