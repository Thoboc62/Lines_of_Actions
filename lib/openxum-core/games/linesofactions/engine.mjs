...
class Engine extends OpenXum.Engine {
    constructor(t, c) {
        ...
        this._count_white_pawn = this._count_nb_pieces_white();
        this._count_black_pawn = this._count_nb_pieces_black();
        //this._verifierPion();
    ...
    _get_possible_move_list(piece) {
        ...
        //console.log("Départ: x= " + x_destination + " y= " + y_destination);
    ...
    _initialize_board() {
        ...
        for(let i = 1; i < 7; i++) {
            ...
            //this._board[3][3] = new Piece(Color.BLACK, new Coordinates(3, 3));
    ...
    identical_neighbour(piece) {
        let PieceX = parseInt(piece.coordinates().x());
        let PieceY = parseInt(piece.coordinates().y());
        let colorPiece = piece.color();

        let NeighbourSameColor = false;
        //declare neighbours

        let pieceDroite = this._board[PieceX + 1][PieceY];
        let pieceInfDroite = this._board[PieceX + 1][PieceY + 1];
        let pieceInf = this._board[PieceX][PieceY + 1];

        let pieceInfGauche;
        if (PieceX >= 1 && PieceY <  7) {
            pieceInfGauche = this._board[PieceX - 1][PieceY + 1];
        }

        let tab_neighbours = new Array(pieceDroite, pieceInfDroite, pieceInf, pieceInfGauche);

        for (let i = 0; i < tab_neighbours.size; ++i) {
            let color_current_neighbours = tab_neighbours[i].color();
            if (typeof tab_neighbours[i] === "object") {
                if (color_current_neighbours === colorPiece) {
                    return true;
                }
            }
            return NeighbourSameColor;
        }
    }
    ...
    testFin() {
        let winBlack = false;
        let winWhite = false;
        let totalNumberBlack = this._count_nb_pieces_black();
        let totalNumberWhite = this._count_nb_pieces_white();
        let cptBlack = 0;
        let cptWhite = 0;
        ...
        for (let v = 0; v < 8; ++v) {
            for (let w = 0; w < 8; ++w) {
                ...
                if (typeof current_case !== "undefined") {
                    // console.log(current_case.color());
                    //console.log(Color.BLACK);
                    if (current_case.color() === Color.BLACK) {
                        ++cptBlack;
                    }
                    if (current_case.color() === Color.WHITE) {
                        ++cptWhite;
                    }
                    if (this.identical_neighbour(current_case) && current_case.color() === Color.BLACK && cptBlack < totalNumberBlack) {
                        winBlack = false;
                    }
                    if (this.identical_neighbour(current_case) && current_case.color() === Color.WHITE && cptWhite < totalNumberWhite) {
                        winBlack = false;
                    }
                    if (cptBlack === totalNumberBlack) {
                        if (this._rest_neighbours_test_black(current_case)) {
                            winWhite = true;
                        }
                    }
                    if (cptWhite === totalNumberWhite) {
                        if (this._rest_neighbours_test_white(current_case)) {
                            winBlack = true;
                        }
                    }
                }
...
//test des dernières destinations possibles pour voir sil y a un voisin
    _rest_neighbours_test_white(pieceCourante) {
        let pieceCouranteX=parseInt(pieceCourante.coordinates().x());
        let pieceCouranteY=parseInt(pieceCourante.coordinates().y());
        let white=Color.WHITE;
        if(this._board[pieceCouranteX+1][pieceCouranteY-1] === "object") {
            if (this._board[pieceCouranteX + 1][pieceCouranteY - 1].color() === white) {
                return true;
            }
        }
        else if(this._board[pieceCouranteX+1][pieceCouranteY] === "object") {
            if (this._board[pieceCouranteX + 1][pieceCouranteY].color() === white) {
                return true;
            }
        }
        else if(this._board[pieceCouranteX-1][pieceCouranteY+1] === "object"){
            if(this._board[pieceCouranteX-1][pieceCouranteY+1].color()===white){
                return true;
            }
        }
    ...
    _rest_neighbours_test_black(pieceCourante){
        let pieceCouranteX=parseInt(pieceCourante.coordinates().x());
        let pieceCouranteY=parseInt(pieceCourante.coordinates().y());
        let black=Color.BLACK;

        if(this._board[pieceCouranteX+1][pieceCouranteY-1] === "object") {
            if (this._board[pieceCouranteX + 1][pieceCouranteY - 1].color() === black) {
                return true;
            }
        }
        if(this._board[pieceCouranteX + 1][pieceCouranteY] === "object") {
            if (this._board[pieceCouranteX + 1][pieceCouranteY].color() === black) {
                return true;
            }
        }
        if (this._board[pieceCouranteX - 1][pieceCouranteY + 1] === "object") {
