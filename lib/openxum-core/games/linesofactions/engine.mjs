...
class Engine extends OpenXum.Engine {
    ...
    _get_possible_move_list(piece) {
        ...
        dernier_y = parseInt(piece.coordinates().y()) + parseInt(this._compterPionsLigne(x_destination));
        ...
        while(!place_trouvee && y_destination >= 0 && y_destination > dernier_y && dernier_y >= 0) {
            let y_suivant = parseInt(y_destination)-parseInt(1);
        ...
        x_destination = piece.coordinates().x();
        y_destination = piece.coordinates().y();
        dernier_x = parseInt(piece.coordinates().x()) + parseInt(this._compterPionsColonne(y_destination));
        parseInt(x_destination); parseInt(y_destination); parseInt(dernier_x); parseInt(taille_grille);
        place_trouvee = false;

        while(!place_trouvee && x_destination < taille_grille && x_destination < dernier_x && dernier_x < taille_grille) {
            let x_suivant = parseInt(x_destination)+parseInt(1);

            //on vérifie si la case suivante est vide
            if(typeof this._board[x_suivant][y_destination] === "undefined") {
                x_destination = x_suivant;
                place_trouvee = false;
                if(x_suivant === dernier_x) {
                    x_destination = x_suivant;
                    place_trouvee = true;
                    break;
                }
            ...
            //ou si elle contient une couleur (number)
            else if(typeof this._board[x_suivant][y_destination] === "object" || typeof this._board[x_suivant][y_destination] === "number") {
                //on vérifie si un pion adverse coupe la route (on ne peut pas l'écraser car la distance à effectuer est plus grande)
                if(this._board[x_suivant][y_destination].color() !== piece.color() && x_suivant !== dernier_x) {
                    place_trouvee = false;
                    break;
                }
        ...
        if(place_trouvee === true && x_destination !== piece.coordinates().x()) {
        ...
        while(!place_trouvee && x_destination >= 0 && x_destination > dernier_x && dernier_x >= 0) {
            let x_suivant = parseInt(x_destination)-parseInt(1);
    ...
    _initialize_board() {
        ...
        this._board[5][5] = new Piece(Color.BLACK, new Coordinates(5,5));
        this._board[4][3] = new Piece(Color.WHITE, new Coordinates(4,3));
