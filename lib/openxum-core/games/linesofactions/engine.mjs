...
class Engine extends OpenXum.Engine {
    ...
    _get_possible_move_list(piece) {
        ...
        console.log("Taille grille: " + taille_grille);

        //positions possibles sur la ligne

            x_destination = parseInt(piece.coordinates().x());
            y_courant = parseInt(piece.coordinates().y());

            //sur la ligne à droite

                //pour chaque case sur la droite (tant qu'on ne rencontre pas un pion adverse)
                for(let y_courant=piece.coordinates().y(); y_courant<taille_grille-1; y_courant=parseInt(y_courant)+1) {
                    let y_destination = parseInt(y_courant)+1;
                    console.log("Case courante: (" + x_destination + "," + y_courant + ")")
                    //console.log("Case suivante: (" + x_destination + "," + y_destination + ")")

                    //on ne doit pas aller plus loin que ce qui est autorisé
                    if(y_destination <= piece.coordinates().y() + this._compterPionsLigne(piece.coordinates().x())) {

                        //si la place suivante ne contient aucun pion
                        if(typeof this._board[piece.coordinates().x()][y_destination] === "undefined") {
                            console.log("INFO: Case suivante vide. On peut continuer à parcourir");
                        }

                        //si la place suivante contient un pion de couleur adverse ET que le y suivant ne sera pas le dernier
                            //on ne peut pas passer au dessus, on doit se placer avant
                        else if(this._board[piece.coordinates().x()][y_destination] !== piece.color()
                            && y_destination < piece.coordinates().y() + this._compterPionsLigne(piece.coordinates().x())) {
                            console.log("INFO: Pièce adverse rencontrée en chemin. On doit se placer avant.");
                            console.log("Destination possible: (" + x_destination + ",", y_courant + ")");
                            break;
                        }

                        //si la place suivante contient un pion de couleur adverse ET que le y suivant sera le dernier possible
                        else if(this._board[piece.coordinates().x()][y_destination] === piece.color()
                            && y_destination === piece.coordinates().y() + this._compterPionsLigne(piece.coordinates().x())) {
                            console.log("INFO: Dernière case possible occupée par un adversaire. Un on l'écraser.");
                        ...
                        console.log("INFO: Distance maximale atteinte");
                        break;
                    }
